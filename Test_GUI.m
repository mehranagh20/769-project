...
% figure();pcshow(ptCloud, VerticalAxis="Y", VerticalAxisDir="Up", ViewPlane="YX");
% %%
% img_gaussian = imgaussfilt(normal, 0.01);
% figure();imshow(img_gaussian);
% 
% %% watershed
% normal = imread('./test8.png');
% Noormal=rgb2gray(normal);
% figure();imshow(Noormal)
% %I1 = imtophat(Noormal, strel('disk', 5));
% %figure();imshow(I1)
% I2 = imadjust(Noormal);
% level = graythresh(I2);
% BW = im2bw(I2, level);
...
maxCameraDepth   = 5;
depth_converted=uint8(rescale(new_depth, 100, 200));
depth_orginal=uint8(rescale(depth,100, 200));
ptCloud = pcfromdepth(depth_orginal,depthScaleFactor, intrinsics, ...
                      ColorImage=rgb, ...
                      DepthRange=[0 maxCameraDepth]);
figure();pcshow(ptCloud, VerticalAxis="Y", VerticalAxisDir="Up", ViewPlane="YX");
