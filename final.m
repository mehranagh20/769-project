...
for cur_i = 1:length(disparities)
    ...
    mask = depth_grad + normal_grad / 5 + (depth_grad > 0.02) / 10;
    mask(mask > 1) = 1;
    ...
    colormap(cm_inferno)
    figure, imshow(rescale(1./fitted, 0, 1))
    colormap(cm_inferno);
    figure, imshow(rescale(1./depth, 0, 1))
    colormap(cm_inferno);
    figure, imshow(rescale(disparity, 0, 1))
    colormap(cm_inferno);
    ...
    rgbImage = ind2rgb(rescale(depth, 0, 1), colormap(cm_inferno));
    colormap(cm_inferno);
    imwrite(rescale(fitted, 0, 1), colormap(cm_inferno), strcat('./data/out/', num2str(cur_i), '_depth_ours', '.png'))
    colormap(cm_inferno);
    % imwrite(rescale(depth, 0, 1), strcat('./data/out/', num2str(cur_i), '_depth', '.png'))
    imwrite(rescale(rgbImage, 0, 1), strcat('./data/out/', num2str(cur_i), '_depth', '.png'))
